...
gen_camb_file($CAMBDefParams,$CAMBParams,$Directory."/".$Prefix,$Omega_Nu,$Omega_B, $Omega_M, $hub, $NS, $A_prim, @Redshifts);
#gen_class_file($classdefparams,$classParams,$Directory."/".$Prefix,$M_Nu,$Omega_B, $Omega_M, $hub, $NS, $A_prim, @Redshifts);
print "Running CAMB...\n";
print_run("$CAMB $CAMBParams");
#print_run("$class $classParams $classprecision");
...
#Class does something weird with the neutrino transfer functions. Use CAMB.
# paramfile newparams output_root omega_nu omega_b omega_cdm hubble redshift
sub gen_class_file{
        my $paramfile=shift;
        my $newparams=shift;
        ...
        while(<$INHAND>){
                ...
                #This is for neutrino decoupling
                s/^\s*[#]*T_ncdm\s*=\s*[\w\/., -]*/T_ncdm= 0.715985,0.715985,0.715985/i;
                ...
                s/^\s*headers\s*=\s*[\w\/.-]*/headers= yes/i;
                s/^\s*YHe\s*=\s*[\w\/.-]*/YHe = 0.24/i;
                #Set initial conditions; scalar_amp is to give sigma_8 = 0.878 at z=0 with nu=0.
                s/^\s*A_s\s*=\s*[\w\/.-]*/A_s = $A_prim/i;
